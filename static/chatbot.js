...
async function handleSend() {  // Send karne ke liye function
    ...
    try {
        ...
        // Link part
        if (data.link) {  // Check if there is a link to show
            const linkDiv = document.createElement('div');
            linkDiv.style.color = 'blue';
            linkDiv.style.textDecoration = 'underline';
            linkDiv.style.cursor = 'pointer';
            linkDiv.innerHTML = `<a href="${data.link}" target="_blank">${data.link}</a>`;  // Assuming data.link has the URL
            chatWindow.appendChild(linkDiv);  // Add the link to the chat window
        }
